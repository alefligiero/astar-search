...
public class OrderedArray {
	...
	public void insertVertex(Vertex v, Vertex atual) {
		...
		// Ordering the array based on the shortest heuristic distance + edge distance (A* distance)
		for (int i = 0; i < vertices.length; i++) {
			for (int j = 0; j < vertices.length; j++) {
				if (vertices[i] != null && vertices[j] != null) {
					...
					if (vertices[i].getDistance() + a.getCost() < vertices[j].getDistance() + b.getCost()) {
